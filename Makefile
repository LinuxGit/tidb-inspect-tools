...
.PHONY: build tidb_exporter tikv_exporter grafana_collector kafka_adapter syslog_adapter tcp_prober clean

build: check test tidb_exporter tikv_exporter grafana_collector kafka_adapter syslog_adapter tcp_prober

tidb_exporter:
	$(GO) build -ldflags '$(LDFLAGS)' -o bin/tidb_exporter tidb_exporter/*.go

tikv_exporter:
	$(GO) build -ldflags '$(LDFLAGS)' -o bin/tikv_exporter tikv_exporter/*.go
